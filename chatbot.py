...
# Page layout
st.set_page_config(page_title="Wafula France Alternative Transportation", layout="wide", page_icon='🇫🇷')
...
st.markdown("# 🚇 " + translate("Wafula France Alternative Transportation Chatbot 🚍", selected_language_code))
    ...
    if input_map['last_active_drawing'] and len(input_map['all_drawings']) >= 2:
        ...
        if len(all_drawings) > 2:
            ...
            if start_loc and dest_loc:
                ...
                elif station_type_value == "TRAIN":
                    nearest_station, closest_dest_station, closest_dest_station_name, station_name, station_type = find_nearest_bus_station(latitude_startloc, longitude_startloc, latitude_destloc, longitude_destloc)
                elif station_type_value == "METRO":
                    nearest_station, closest_dest_station, closest_dest_station_name, station_name, station_type = find_nearest_bus_station(latitude_startloc, longitude_startloc, latitude_destloc, longitude_destloc)
                elif station_type_value == "TRAMWAY":
                    nearest_station, closest_dest_station, closest_dest_station_name, station_name, station_type = find_nearest_bus_station(latitude_startloc, longitude_startloc, latitude_destloc, longitude_destloc)    
                ...
                if station_type_value == "VELIB":
                    ...
                    chatbot_response(directions_input_station, transport_mean_option, station_name, 'TRAIN', station_type, 'inconnu') 
                elif station_type_value == "METRO":
                    chatbot_response(directions_input_station, transport_mean_option, station_name, 'METRO', station_type, 'inconnu') 
                elif station_type_value == "TRAMWAY":
                    chatbot_response(directions_input_station, transport_mean_option, station_name, 'TRAIN', station_type, 'inconnu') 
